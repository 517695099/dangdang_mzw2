  ...
  <div align=center style="font-size:25px">订单详细信息</div><hr/>
  	 <table bordercolor='#898E90' align='center' border='3px' cellpadding='5px' cellspacing="0px">
          ...
          <c:forEach items="${order.items}" var="book">
			<tr align='center'> 
				 <td>${book.name}</td>
				 <td style="width: 100px;"><img style="width:32px;height: 20px" src="${pageContext.request.contextPath}/back/img/${book.cover}"></td>
				 <td style="width: 180px;">${book.price}</td>
				 <td style="width: 200px;">${book.dprice}</td>
				 <td style="width: 100px;">${book.count}</td>
			</tr>
		</c:forEach>
     </table> 
